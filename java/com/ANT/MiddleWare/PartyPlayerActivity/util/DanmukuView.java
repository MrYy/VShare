...
public class DanmukuView extends View{
    private static final String TAG = "DanmukuView";
    ...
    private HashMap<Integer,ArrayList<IDanmukuItem>> mChannelMap;
    private final java.util.Deque<IDanmukuItem> mWaitingItems = new LinkedList<>();
    ...
    public DanmukuView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public DanmukuView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        mContext = context;
        TypedArray a = mContext.obtainStyledAttributes(attrs, R.styleable.DanmukuView, 0, 0);
        mMaxRow = a.getInteger(R.styleable.DanmukuView_max_row, 1);
        mPickItemInterval = a.getInteger(R.styleable.DanmukuView_pick_interval, 1000);
        mMaxRunningPerRow = a.getInteger(R.styleable.DanmukuView_max_running_per_row, 1);
        mShowDebug = a.getBoolean(R.styleable.DanmukuView_show_debug, false);
        mStartYOffset = a.getFloat(R.styleable.DanmukuView_start_Y_offset, 0.1f);
        mEndYOffset = a.getFloat(R.styleable.DanmukuView_end_Y_offset, 0.9f);
        a.recycle();
    ...
    private void initChannelMap(){
        ...
        for (int i = 0; i < mMaxRow; i++) {
            ArrayList<IDanmukuItem> runningRow= new ArrayList<IDanmukuItem>(mMaxRunningPerRow);
        ...
        if (status == STATUS_RUNNING) {
            try {
                ...
                for (int i = 0; i < mChannelMap.size(); i++) {
                    ArrayList<IDanmukuItem> list = mChannelMap.get(i);
                    for (Iterator<IDanmukuItem> it = list.iterator(); it.hasNext(); ) {
                        IDanmukuItem item = it.next();
                        if (item.isOut()) {
                            it.remove();
                        } else {
                            item.doDraw(canvas);
...
//                    Log.d(TAG, "start pick new item..");
                    IDanmukuItem di = mWaitingItems.pollFirst();
                    if (di != null) {
...
//                                Log.d(TAG, "Not find vacant channel, add it back");
    ...
    private int findVacant(IDanmukuItem item) {
        try {//fix NPT exception
            ...
            for (int i = 0; i < mMaxRow; i++) {
                ArrayList<IDanmukuItem> list = mChannelMap.get((i + ind) % mMaxRow);
                if (list.size() > mMaxRunningPerRow) {//每个弹道最多mMaxRunning个弹幕
                    continue;
                }
                IDanmukuItem di = list.get(list.size() - 1);
                if (!item.willHit(di)) {
...
//    }
    ...
    public void addItem(IDanmukuItem item) {
        synchronized (mWaitingItems) {
            this.mWaitingItems.add(item);
        }
    }

    public void addItemToHead(IDanmukuItem item) {
        synchronized (mWaitingItems) {
    ...
    /**是否新建后台线程来执行添加任务*/
    public void addItem(final List<IDanmukuItem> list, boolean backgroundLoad) {
        if (backgroundLoad) {
            new Thread(){
                @Override
                public void run() {
